...
int f(int* dp, int len, int* ex, int index){
    //cout << len << endl;
    if(dp[index] == 0){
        int value = a[index];
        int ret = 1;
        int m = a[len-1];
        for(int i = value*2; i<=m; i+=value){
            if(ex[i] == 0){
                continue;
            }
            else{
                int newIndex = ex[i]-1;
                ret = max(ret, f(dp, len, ex, newIndex)+1);
            }
        }
        dp[index] = ret;
        return dp[index];
...
std::ios::sync_with_stdio(false);
    ...
    for(int i = 0; i<n; i++){
        ...
        bool q = 0;
        for(int j = i-1; j>=0; j--){
            if(a[j] != 0 && (a[i] % a[j]) == 0 && (a[i]/a[j]) == b){
                cnt++;
            }
            if(a[i] == a[j]){
                before[i] = before[j] + cnt;
                q = 1;
                break;
            }
        }
